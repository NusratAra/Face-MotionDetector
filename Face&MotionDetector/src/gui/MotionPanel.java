...
public class MotionPanel extends JPanel implements Runnable {
    ...
    ////////////////
    //create folder to save images of motion detection
      public void doF1() {
        File mf = new File("MotionDetection");
        if (!mf.isDirectory()) {
            mf.mkdir();
        }
    }
     ...
     */ {
        ...
        while (isRunning) {
            ...
            if ((pt = md.getCOG()) != null) {    // get new COG
                ...
                cogPoint = pt;
                reportCOGChanges(cogPoint, prevCogPoint, grabber);
            }
    ...
    private void reportCOGChanges(Point cogPoint, Point prevCogPoint, FrameGrabber grabber) // compare cogPoint and prevCogPoint
    {
        ...
        if (distMoved > MIN_MOVE_REPORT) {
            ...
            System.out.println("  Dist moved: " + distMoved + "; angle: " + angle);
                ...
                doF1();
                IplImage img1=  picGrab(grabber, CAMERA_ID);
                long ct1 = System.currentTimeMillis();
                String aa = "MotionDetection/save" +ct1+ ".jpg";
                cvSaveImage(aa, img1);
    ...
    public void paintComponent(Graphics g) /* Draw the image, the movement crosshairs, and the 
    ...
    private void drawCrosshairs(Graphics g, int xCenter, int yCenter) // draw crosshairs graphic or a red circle
    {
        if (crosshairs != null) {
